...
def resolve_paths(run_dir: Path, config_path: str | None):
    ...
    return out_dir, ckpt_dir, config_path
...
def make_dataset(cfg_json, batch_size):
    points = cfg_json["spectrum"]["total_points"]
    ...
    ds = (
        tf.data.Dataset.from_generator(
            lambda: generator(cfg_json),   # <- structured JSON, generator normalizes internally
...
def main(
    stage: str = "stage1",
    checkpoint_root: str = "checkpoints_test",
    batch_size: int = 10,
    ...
    out_dir, ckpt_dir, config_path = resolve_paths(run_dir, config_path=None)



    cfg_json = load_config(config_path)          # structured JSON
    points = cfg_json["spectrum"]["total_points"]

    # all outputs go here:
    checkpoint_root = str(ckpt_dir)
    summary_stage1_path = str(out_dir / "final_net_stage1.txt")
    summary_stage2_path = str(out_dir / "final_net_stage2.txt")
    lr_log_path = str(out_dir / "learning_rate_log.csv")
    ...
    lr_logger = LearningRateTracker(log_file=lr_log_path)
    ...
    if stage in ("stage1", "both"):
        ...
        save_model_summary(core, summary_stage1_path)
    ...
    if stage in ("stage2", "both"):
        ...
        save_model_summary(core, summary_stage2_path)
